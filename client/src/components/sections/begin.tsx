import { Settings, Sparkles, Cpu, Mic, MessageCircle, Server } from "lucide-react";
import { motion } from "framer-motion";

const Begin = ({ prefixBg }: { prefixBg: string }) => {
  return (
    <section className={`pt-64 pb-12 px-6 sm:px-10 ${prefixBg}`}>
      <div className="max-w-6xl mx-auto text-center px-4 sm:px-8">
        ...
        <div className="mb-16">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-[#F1F1F1] to-[#FAFAFA] text-[#5B3EFF] px-5 py-2 rounded-full text-sm sm:text-base font-medium mb-6 shadow-sm backdrop-blur-md bg-opacity-30">
            <Settings size={18} />
            <span>Разработка LLM, AI и backend решений под ваш бизнес</span>
          </div>
          ...
          <p className="text-lg sm:text-xl text-gray-700 max-w-3xl mx-auto leading-relaxed">
            Молодая IT-компания, специализирующаяся на заказной разработке высокотехнологичного программного обеспечения для малого и среднего бизнеса.
          </p>
        </div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="mb-20 flex flex-col md:flex-row gap-8 justify-center"
        >
          <div className="md:flex-[3] p-12 rounded-3xl shadow-lg bg-white/70 dark:bg-gray-800/70 text-gray-700 dark:text-gray-300 backdrop-blur-md border-8 border-gray-100 transition-colors duration-500">
...
const Feature = ({ icon, text }: { icon: React.ReactNode; text: string }) => (
  <div className="flex items-center gap-3 bg-white/90 dark:bg-gray-700 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow transition-colors duration-300">
